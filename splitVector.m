...
% Date Started: 7/7/14
% Filename: splitVector.m
%*************************************************************************%

function [output] = splitVector(input,index,upperLimits)

colCount = zeros(1,length(upperLimits));
...
for counter = 1:1:length(input)
    for col = 1:1:length(colCount)
        if col > 1
            if index(counter) >= upperLimits(col-1) && index(counter) < upperLimits(col)
